...
function microphoneStream(encoding, sampleRateHertz, languageCode) {
  ...
  const recognizeStream = client
    ...
    .on('data', data =>
      process.stdout.write(!startEx ? (data.results[0] && data.results[0].alternatives[0]
          ? (data.results[0].alternatives[0].transcript.includes("begin exercis")
            ?  startExercises() : `\n\nYOU DIDNT SAY START\n as you said: ${data.results[0].alternatives[0].transcript}`)
          : `\n\nReached transcription time limit, press Ctrl+C\n`) : (data.results[0] && data.results[0].alternatives[0]
          ? (data.results[0].alternatives[0].transcript.includes("finish exercis") 
            ?  stopExercises() : `\n\nYOU DIDNT SAY STOP\n as you said: ${data.results[0].alternatives[0].transcript}`)
          : `\n\nReached transcription time limit, press Ctrl+C\n`)
...
//     .on('error', console.error)
//     .on('data', data =>
//       process.stdout.write( { if (data.results[0] && data.results[0].alternatives[0]){
//             if (!startEx){ 
//               return (data.results[0].alternatives[0].transcript == "start" 
//               ?  startExercises() : `\n\nYOU DIDNT SAY START\n as you said: ${data.results[0].alternatives[0].transcript}`)

//           } else {
//             return (data.results[0].alternatives[0].transcript == "stop" 
//               ?  stopExercises() : `\n\nYOU DIDNT SAY START\n as you said: ${data.results[0].alternatives[0].transcript}`)
//           }
